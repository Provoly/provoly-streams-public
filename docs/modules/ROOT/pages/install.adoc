...
* update the parent `icon:file[] pom.xml` by adding the new module name
* update the coverage `icon:file[] pom.xml` by adding the new module name
* update `IMAGES_MAP` environnement variable in  `icon:file[] .github/build.yaml` by adding the module name and its image name.